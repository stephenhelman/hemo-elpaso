...
import { applicationFormTranslation } from "@/translation/portalAssistance";
...
export default function ApplicationForm({ patient, locale }: Props) {
  const router = useRouter();
  const t = applicationFormTranslation[locale];
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-neutral-200 p-6">
        ...
        <p className="text-sm text-neutral-600 mb-4">{t.uploadInstructions}</p>
        ...
        <div className="mb-4">
          <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-neutral-300 rounded-lg cursor-pointer hover:border-primary hover:bg-primary-50 transition-colors">
            ...
            <span className="text-sm text-neutral-600">{t.clickToUpload}</span>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-6">
        <h3 className="font-semibold text-amber-900 mb-2">{t.importantInfo}</h3>
        ...
        >
          {loading ? (
            <>
              ...
              {uploadingFiles ? t.uploadingFiles : t.submitting}
            </>
          ) : (
            t.submit
          )}
